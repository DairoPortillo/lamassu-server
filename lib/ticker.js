...
const bitget = require('./plugins/ticker/bitget')
...
const getFallbackTicker = ticker =>
  _.difference(['bitpay', 'kraken', 'bitstamp', 'bitget'], [ticker])[0]
...
function buildTicker (fiatCode, cryptoCode, tickerName) {
  ...
  if (tickerName === 'bitget') return bitget.ticker(fiatCode, cryptoCode)
